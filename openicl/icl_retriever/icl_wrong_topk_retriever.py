...
class WrongTopkRetriever(BaseRetriever):
    ...
    def knn_search(self, ice_num):
        ...
        if not self.rev:
            ...
            for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
                idx = entry['metadata']['id']
                embed = np.expand_dims(entry['embed'], axis=0)
                near_ids = self.index.search(embed, len(self.index_ds))[1][0][-ice_num:].tolist()
                rtr_idx_list[idx] = near_ids
    ...
    def generate_reverse_mapping_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
    ...
    def generate_reverse_fix_mapping_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
    ...
    def generate_reverse_dyn_mapping_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
        ...
        # get the demo and the idx of the demo
        demo_label_idxs = dict()
        labels_num = [0 for i in range(len(self.labels))]
        ...
        for idx in range(len(demo_label)):
            ...
            labels_num[demo_label[idx]] += 1

        label_values = list(sorted(enumerate(labels_num), key=lambda x: x[1]))
        new_label_order = [label_value[0] for label_value in label_values]

        new_mapping = {new_label_order[i]: new_label_order[(i + 1) % len(self.labels)] for i in range(len(new_label_order))}
        ...
        for label in demo_label:
            #key = random.sample(offsets, 1)[0]
            new_demo_text.append(random_demo_text[new_mapping[label]].pop())
    ...
    def generate_reverse_true_dyn_mapping_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
        ...
        label_values = list(sorted(enumerate(labels_num), key=lambda x: x[1], reverse=True))
        new_label_order = [label_value[0] for label_value in label_values]
    ...
    def generate_reverse_input_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
        ...
        for label in demo_label:
            new_demo_text.append(random_demo_text[label].pop())
    ...
    def generate_reverse_label_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
        ...
        for label in demo_label:
            key = random.sample(offsets,1)[0]
            new_demo_label.append((label + key) % num_label)

        #print(new_demo_text)
        generated_ice_list = [ice_template.generate_ice_item(demo_text[idx], new_demo_label[idx]) for idx in range(len(demo_label))]
        generated_ice = self.ice_separator.join(generated_ice_list) + self.ice_eos_token
        return generated_ice

    def generate_reverse_label_seed_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None, seed: int=1) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
    ...
    def generate_reverse_label_only_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
        ...
        #print(new_demo_text)
        generated_ice_list = [ice_template.generate_ice_item(new_demo_text[idx], new_demo_label[idx]) for idx in range(len(demo_label))]
        generated_ice = self.ice_separator.join(generated_ice_list) + self.ice_eos_token
        return generated_ice

    def generate_reverse_label_only_seed_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None, seed: int=1) -> str:
        generated_ice_list = []
        dr = self.dataset_reader
